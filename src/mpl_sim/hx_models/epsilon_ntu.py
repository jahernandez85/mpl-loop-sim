...
class EpsilonNTUModel(HeatExchangerModel):
        ...
        if req.dp_primary is not None:
            if req.dp_primary_is_two_phase:
                ...
                if not math.isfinite(raw_dP_gradient):
                    raise ValueError(
                        f"EpsilonNTUModel: two-phase DP gradient must be finite; "
                ...
                if not math.isfinite(raw_dP):
                    raise ValueError(
                        f"EpsilonNTUModel: DP correlation output must be finite; " f"got {raw_dP!r}"
                    )
    ...
    def _solve_fixed_wall_temp(
        ...
        # DP path — identical dispatch pattern to FixedHeatRate and SinkInletTempAndFlow.
        raw_dP = 0.0
        if req.dp_primary is not None:
            if req.dp_primary_is_two_phase:
                dp_inp = self._build_two_phase_dp_input(req)
    ...
    def _solve_ambient_coupling(
        ...
        # DP path — optional; identical dispatch pattern to other BC paths.
        raw_dP = 0.0
        if req.dp_primary is not None:
            if req.dp_primary_is_two_phase:
                dp_inp = self._build_two_phase_dp_input(req)
    ...
    def _build_two_phase_dp_input(self, req: HXSolveRequest) -> TwoPhaseDPInput:
        ...
        """
        gs = req.geom_scalars
        ctx = "EpsilonNTUModel._build_two_phase_dp_input"
        G = _require_scalar(gs, "G", ctx)
        if G <= 0.0:
            raise ValueError(f"EpsilonNTUModel: geom_scalars['G'] must be > 0; got {G!r}")
        x_val = _require_scalar(gs, "x", ctx)
        ...
        for key in ("rho_l", "rho_v", "mu_l", "mu_v"):
            ...
            if val <= 0.0:
                raise ValueError(f"EpsilonNTUModel: geom_scalars[{key!r}] must be > 0; got {val!r}")
