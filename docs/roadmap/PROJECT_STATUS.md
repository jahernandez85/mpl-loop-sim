...
| **Branch** | `phase-15h-a-structural-residual-diagnostics` |
| **Stage** | Block 15H-A - Explicit Residual/Unknown Structural Diagnostics MVP: independently audited and approved with one minor documentation wording fix |
| **Completed phase** | **Block 15G-C - Explicit Residual Blueprint Workflow Closeout / Acceptance MVP** |
| **Previous completed phase** | Block 15G-B - Explicit Residual Blueprint Selection Workflow Integration MVP |
...
| **Branch status** | **Block 15H-A implemented on `phase-15h-a-structural-residual-diagnostics`, independently audited, and ready for merge.** |
| **Current active phase** | **Block 15H-A — Explicit Residual/Unknown Structural Diagnostics MVP (checkpoint complete and audited)** |
| **Next immediate slice** | Post-15H-A: richer physical residual assembly; production component adapters; property/correlation/HX-backed closures; rank/Jacobian diagnostics if explicitly approved; physically predictive solves — all deferred to future blocks |
| **Baseline before this block** | Block 15G-C merged baseline (post-15G audit): 7402 full-suite tests; 3541 network tests. |
| **Test status** | **7468 full-suite tests and 3607 network tests passed after Block 15H-A. Block 15H-A added 66 tests (52 unit + 14 integration) over the 7402-test post-15G baseline. No failures, errors, skips, or deselections reported in the executed quiet runs.** |
| **Lint status** | **Clean. `ruff check src tests examples` -> All checks passed.** |
| **Format status** | **Clean. `black --check --no-cache src tests examples` -> 237 files unchanged.** |
...
| **Date** | 2026-06-30 |
| **Updated by** | Codex |
| **Status note** | Post-15G baseline independently audited on `audit/post-15g-baseline`; approved with no critical or major findings. The audit confirmed 15F is complete within configurable algebraic residual MVP scope and 15G is complete within explicit residual blueprint workflow MVP scope. No runtime feature changes were made. The accepted 15G stack remains user-declared: explicit scenario build result, explicit residual blueprints, and optional explicit unknown values; evaluation is optional and flows only through existing `CONFIGURABLE_ALGEBRAIC` selection. No blueprint, residual, or closure inference from roles/topology/component types/graph edges, no automatic generation, no property/correlation/HX-backed execution, no production component execution, no `SystemState`, no `FluidState`, no generic `solve(network)`, no `NetworkGraph.solve()`, and no solve path was added to the 15G workflow. Verified post-15G counts: 68 Block 15G-C closeout tests, 57 Block 15G-B regressions, 180 Block 15G-A regressions, 90 15F-C regressions, 53 15F-B regressions, 180 15F-A regressions, 60 production-contract tests, 3541 network tests, and 7402 full-suite tests passed in successful cache-disabled validation runs. Full-suite arithmetic matched `7334 + 68 = 7402`; network-suite arithmetic matched `3473 + 68 = 3541`. No pytest temp/cache permission retry was needed during the post-15G baseline audit. All six examples, Ruff, Black (234 files unchanged), and diff checks passed; all six known production classes remain `NO_CONTRIBUTE_METHOD`. See `POST_15G_BASELINE_AUDIT.md`. |
